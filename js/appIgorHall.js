...
(function initEasterEggs(){
    ...
    // ── Helper: rastrear desbloqueados en localStorage ──────────────────────
    const EGG_STORAGE_KEY = "igorhall_eggs";
    function getUnlockedEggs(){
        try { return JSON.parse(localStorage.getItem(EGG_STORAGE_KEY) || "[]"); } catch(e){ return []; }
    }
    function markEggUnlocked(eggId){
        if(!eggId) return;
        const unlocked = getUnlockedEggs();
        if(!unlocked.includes(eggId)){
            unlocked.push(eggId);
            try { localStorage.setItem(EGG_STORAGE_KEY, JSON.stringify(unlocked)); } catch(e){}
            updateEggIndex();
        }
    }
    function updateEggIndex(){
        const unlocked = getUnlockedEggs();
        const total = document.querySelectorAll(".egg-item[data-egg-id]").length;
        document.querySelectorAll(".egg-item[data-egg-id]").forEach(item => {
            const id = item.dataset.eggId;
            if(unlocked.includes(id)){
                item.classList.add("unlocked");
                const hint = item.querySelector(".egg-item-hint");
                if(hint) hint.textContent = "✓ DESBLOQUEADO";
            }
        });
        const countText = document.getElementById("eggCountText");
        const countBar  = document.getElementById("eggCountBar");
        if(countText) countText.textContent = `${unlocked.length} / ${total} desbloqueados`;
        if(countBar)  countBar.style.width  = `${(unlocked.length / total) * 100}%`;
    }

    // ── Helper: mostrar overlay y cerrarlo ────────────────────────────────────
    function showEgg(id, eggId = "", duration = 4000){
        const el = document.getElementById(id);
        if(!el) return;
        document.querySelectorAll(".easter-overlay.active").forEach(o => o.classList.remove("active"));
        el.classList.add("active");
        el.setAttribute("aria-hidden","false");
        if(eggId) markEggUnlocked(eggId);
    ...
    // Cargar estado del índice al iniciar
    updateEggIndex();
    ...
    document.addEventListener("keydown", e => {
        ...
        if(konamiPos === konamiSeq.length){
            konamiPos = 0;
            showEgg("konamiFlash","konami",3500);
    ...
    document.querySelector(".title")?.addEventListener("click", () => {
        ...
        if(tapCount >= 4){ tapCount = 0; showEgg("konamiFlash","konami",3500); }
    });
    ...
    document.addEventListener("keydown", e => {
        if(e.key.length === 1){
            ...
            if(typed.includes("darkspawn"))  { typed=""; showEgg("eggDragonAge","dragonage",5000); }
            if(typed.includes("inquisition")){ typed=""; showEgg("eggDragonAge","dragonage",5000); }
            if(typed.includes("solas"))      { typed=""; showEgg("eggDragonAge","dragonage",5000); }
            if(typed.includes("dice"))       { typed=""; showEgg("eggBaldur","baldur",5000);    }
            if(typed.includes("baldur"))     { typed=""; showEgg("eggBaldur","baldur",5000);    }
            if(typed.includes("shadowheart")){ typed=""; showEgg("eggBaldur","baldur",5000);    }
            if(typed.includes("laezel"))     { typed=""; showEgg("eggBaldur","baldur",5000);    }
            if(typed.includes("arthur"))     { typed=""; showEgg("eggRedDead","reddead",5000);  }
            if(typed.includes("reddead"))    { typed=""; showEgg("eggRedDead","reddead",5000);  }
            if(typed.includes("vault"))      { typed=""; showEgg("eggFallout","fallout",5000);  }
            if(typed.includes("fallout"))    { typed=""; showEgg("eggFallout","fallout",5000);  }
            if(typed.includes("special"))    { typed=""; showEgg("eggFallout","fallout",5000);  }
            // Metal Gear Solid
            if(typed.includes("snake"))      { typed=""; showEgg("eggMetalGear","metalgear",5000); }
            if(typed.includes("codec"))      { typed=""; showEgg("eggMetalGear","metalgear",5000); }
            if(typed.includes("solidsnake")) { typed=""; showEgg("eggMetalGear","metalgear",5000); }
            if(typed.includes("foxhound"))   { typed=""; showEgg("eggMetalGear","metalgear",5000); }
            // The Witcher
            if(typed.includes("witcher"))    { typed=""; showEgg("eggWitcher","witcher",5000);     }
            if(typed.includes("geralt"))     { typed=""; showEgg("eggWitcher","witcher",5000);     }
            if(typed.includes("gwent"))      { typed=""; showEgg("eggWitcher","witcher",5000);     }
            // One Piece
            if(typed.includes("nakama"))     { typed=""; showEgg("eggOnePiece","onepiece",5000);   }
            if(typed.includes("luffy"))      { typed=""; showEgg("eggOnePiece","onepiece",5000);   }
            if(typed.includes("onepiece"))   { typed=""; showEgg("eggOnePiece","onepiece",5000);   }
        }
    });
